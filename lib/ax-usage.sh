...
import json, sys, time
json.dump({'ts': time.time(), 'pct': int(sys.argv[2]), 'resets': sys.argv[3]},
          open(sys.argv[1], 'w'))
PY
}
...
try:
  ...
  if p is None: sys.exit(1)
  print(int(round(float(p))))
  print(data.get('fiveHourResetsAt') or '')
except Exception: sys.exit(1)
PY
}
...
ax_get_usage() {
  ...
  if out=$(_ax_usage_read_local_cache 2>/dev/null); then
    FIVE_HOUR_PERCENT=$(printf '%s' "$out" | head -1)
    FIVE_HOUR_RESETS_AT=$(printf '%s' "$out" | tail -1)
    return 0
  fi
  ...
  if [ -n "$token" ]; then
    ...
    if [ -n "$response" ]; then
      ...
      if [ -n "$parsed" ]; then
        FIVE_HOUR_PERCENT=$(printf '%s' "$parsed" | head -1)
        FIVE_HOUR_RESETS_AT=$(printf '%s' "$parsed" | tail -1)
