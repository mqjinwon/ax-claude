...
```bash
git clone https://github.com/<your-username>/ax ~/.ax
~/.ax/setup
```

`setup`이 자동으로 처리하는 것:
- `/ax` skill symlink 생성: `~/.claude/skills/ax/SKILL.md`
- SessionEnd hook 등록: `~/.claude/settings.json`
- 모든 `.sh` 스크립트에 실행 권한 부여
...
### GitHub에 push된 경우 one-liner 설치

```bash
git clone https://github.com/<your-username>/ax ~/.ax && ~/.ax/setup && echo 'export PATH="$HOME/.ax/bin:$PATH"' >> ~/.bashrc
```
...
`~/.ax/`가 git repo인 경우 `git pull` 후 `setup` 재실행.
...
제거 대상:
- `~/.claude/skills/ax/SKILL.md` symlink
- `~/.claude/settings.json`의 ax-ingest.sh hook 엔트리

**보존 대상**: `~/.ax/` 디렉토리 및 모든 프로젝트 `.ax/` 데이터
