...
설치 후 새 프로젝트에서 한 번만 실행:
```bash
ax init
```

SessionEnd hook과 `/ax` skill은 **plugin system이 자동 등록**.
python3 + pyyaml이 없으면 `/ax` routing은 manual fallback으로 동작.

> **이미 `/plugin marketplace add`를 실행했다면**: 마켓플레이스를 제거 후 재추가해야 합니다.
> Claude Code CLI에서 `/plugin marketplace remove ax-claude` 후 다시 위 명령어를 실행하세요.
...
├── hooks/
│   └── hooks.json          # SessionEnd hook 정의 (plugin system 등록용)
├── lib/
